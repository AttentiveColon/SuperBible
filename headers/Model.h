...
namespace SB2
{
	...
	void Model::OnDraw() {
		for (const auto& node_index : m_scenes[m_current_scene].m_node_indices) {
			glUniformMatrix4fv(3, 1, GL_FALSE, glm::value_ptr(m_nodes[node_index].m_trs_matrix));
			m_meshes[m_nodes[node_index].m_mesh_index].OnDraw();
			for (const auto& child_node_index : m_nodes[node_index].m_children_nodes) {
				glUniformMatrix4fv(3, 1, GL_FALSE, glm::value_ptr(m_nodes[child_node_index].m_trs_matrix * m_nodes[node_index].m_trs_matrix));
				m_meshes[m_nodes[child_node_index].m_mesh_index].OnDraw();
			}
		}
	}
}
