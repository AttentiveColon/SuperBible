...
void main()
{
	...
	diffuse_color *= 255.0;
	ivec3 divisions = ivec3(diffuse_color / div); 
	diffuse_color = vec3(divisions.r * scaling_amount, divisions.g * scaling_amount, divisions.b * scaling_amount);
	...
	diff *= 255.0;
	int division = int(diff / div);
	diff = division * scaling_amount;
